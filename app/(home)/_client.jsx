...
// app/_client.jsx  (الصفحة الرئيسية — client component)
import React, { useState, useEffect } from 'react';
import Image from 'next/image';
import Link from 'next/link';
import { ArrowRight, ExternalLink, CheckCircle, Star, MessageCircle } from 'lucide-react';
...
import ServicesGrid from '../(components)/(realComponents)/ServicesGrid';  // ← الكومبوننت المشترك

const CodeLegacyClient = ({ services, testimonials, webServices }) => {
  ...
  const whatsappMessage = `السلام عليكم ورحمة الله وبركاته \n\nعايز أطور موقع إلكتروني خاص بيا\n\nممكن تعرفوني بالأسعار والباقات المتاحة؟`;
  const whatsappURL = `https://wa.me/${whatsappPhone}?text=${encodeURIComponent(whatsappMessage)}`;
  const handleWhatsAppClick = (e) => { e.preventDefault(); window.open(whatsappURL, '_blank'); };
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center animate-[fadeInUp_1s_ease_0.6s_both] px-2 mt-[75px]">
<button
  onClick={handleWhatsAppClick}
  className="group bg-gradient-to-r from-blue-500 to-purple-500 px-6 sm:px-8 py-3 sm:py-4 rounded-full text-base sm:text-lg font-semibold hover:from-blue-600 hover:to-purple-600 transition-all transform hover:scale-105 flex items-center justify-center gap-2 shadow-2xl w-full sm:w-auto"
>
  ابدأ رحلتك معنا
  <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
        ...
        <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-none">
          <svg className="relative block w-[calc(100%+1.3px)] h-[60px] sm:h-[80px] md:h-[120px]" viewBox="0 24 150 28" preserveAspectRatio="none">
            <defs>
      ...
      {/* ═══════════════════════════════════════
          Portfolio Section
      ═══════════════════════════════════════ */}
      <PortfolioSection title="بعض من أعمالنا السابقة" showCtaButton={false} />
      <div className="text-center pb-12 sm:pb-16 lg:pb-20 bg-white -mt-8 sm:-mt-12">
        <Link
      ...
      {/* ═══════════════════════════════════════
          Services Grid — الكومبوننت المشترك
          showViewAll=true يُظهر زر "استكشف جميع خدماتنا"
      ═══════════════════════════════════════ */}
      <ServicesGrid
        services={services}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 sm:mb-16">
            ...
            <p className="text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed px-2">
              رحلتنا في بناء المستقبل الرقمي، حيث نجمع بين الإبداع والتكنولوجيا لتقديم مواقع ويب تحدث فرقًا.
            </p>
          </div>
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div className="relative order-2 lg:order-1">
              <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg border border-blue-200/50 hover:shadow-xl transition-shadow duration-300">
                <h3 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-3 sm:mb-4">من نحن؟</h3>
                <p className="text-base sm:text-lg text-gray-600 mb-4 sm:mb-6 leading-relaxed">
                  CodeLegacy هي شركة رائدة في مجال تطوير الويب، تأسست برؤية لتحويل الأفكار إلى مواقع ويب مبتكرة. نحن فريق من الخبراء المتحمسين الذين يجمعون بين الإبداع التقني والخبرة العملية لتقديم مواقع ويب تتجاوز التوقعات.
                </p>
                <p className="text-base sm:text-lg text-gray-600 mb-6 sm:mb-8 leading-relaxed">
                  مهمتنا هي تمكين الشركات من تحقيق طموحاتها الرقمية من خلال مواقع ويب مخصصة تدعم النمو وتعزز التنافسية في عالم الويب المتسارع.
                </p>
                <Link
                  href="/about"
            ...
            <div className="space-y-4 sm:space-y-6 order-1 lg:order-2">
              <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-md border border-blue-200/50">
                <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">ما يميزنا</h3>
                <div className="space-y-3 sm:space-y-4">
                  {[
                    ...
                    <div key={index} className="flex items-start space-x-2 sm:space-x-3 rtl:space-x-reverse">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {[
                  ...
                  <div key={index} className="text-center p-3 sm:p-4 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-xl hover:scale-105 transition-transform duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {webServices.map((service, index) => (
              <div key={index} className="group bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-blue-200/50 hover:shadow-xl transition-all duration-300 text-center">
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50" id="clients">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 sm:mb-16">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-gray-600 max-w-3xl mx-auto px-2">
              آراء عملائنا هي شهادتنا الحقيقية على جودة العمل والالتزام بتقديم الأفضل
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {testimonials.map((testimonial, index) => (
              <div key={index} className="bg-white p-4 sm:p-6 rounded-2xl shadow-md border border-gray-100">
                ...
                <div className="flex items-center">
                  <Image src={testimonial.avatar} alt={testimonial.name} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover" width={50} height={50} />
      ...
      <style jsx>{`
        ...
        @media (max-width: 640px) {
          ...
          .animate-\[float_6s_ease-in-out_infinite_4s\] { animation-duration: 8s; }
        }
      `}</style>
    </>
  );
